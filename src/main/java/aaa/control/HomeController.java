...
public class HomeController {
	...
	String scope(Model mm, HttpSession session) {
		...
		session.setAttribute("ph1", new HandPhone("req_Ph1",10));
		session.setAttribute("ph2", new HandPhone("req_Ph2",20));
	...
	String calc(Model mm, HttpSession session) {
		System.out.println("/basic/calc 실행");
		mm.addAttribute("aaa",456);
		mm.addAttribute("ph",new HandPhone("은하수",77));
		mm.addAttribute("st",new Stud("김탁구",88,80,80));
		
		return "basic/calc";
	}
	
	@RequestMapping("/basic/control")
	String control(Model mm, HttpSession session) {
	...
	String link(Model mm, HttpSession session) {
	...
	@PostMapping(value = "/basic/joinGo")
